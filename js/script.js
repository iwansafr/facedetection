  ...
  imageUpload.addEventListener("change", async () => {
    ...
    result.forEach((results, index) => {
      ...
      contentImage += `<p style="position:absolute;color:#fff;background:red;width:${Math.round(
        ...
        box.x
      }px;border:solid;border-color:red;margin-top: 0px; font-size: 12px;">${results.toString()}</p>`;
      if (addData) {
...
async function loadLableImages() {
  ...
  return Promise.all(
    data.map(async (items) => {
      let description = [];
      const img = await faceapi.fetchImage(`images/${items.image}`);
      const detection = await faceapi
